...
import { getVenues, getRankingByHirer, saveRankingForHirer } from '../../utils/storage'
import type { Venue } from '../../types'

export default function VenueRankings() {
  const { currentUser } = useAuth()
  const allVenues = getVenues().filter(v => !v.isBlocked)
  ...
  const [ranked, setRanked] = useState<Venue[]>(() => {
    if (existingRanking?.rankings.length) {
      return existingRanking.rankings
        .sort((a, b) => a.rank - b.rank)
        .map(r => allVenues.find(v => v.id === r.venueId))
        .filter(Boolean) as Venue[]
    }
    return []
  })
  const [pool, setPool] = useState<Venue[]>(() => {
    const rankedIds = new Set(existingRanking?.rankings.map(r => r.venueId) ?? [])
    return allVenues.filter(v => !rankedIds.has(v.id))
  })
