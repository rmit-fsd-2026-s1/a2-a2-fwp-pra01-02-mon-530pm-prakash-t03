/**
 * VENUE VENDORS CLIENT APP - SIGNIN.TSX
 * 
...
export default function SignIn() {
  ...
  // Retrieve logout success message from router navigation state if present
  useEffect(() => {
    if (location.state?.logoutMessage) {
      setMessage({ type: 'success', text: location.state.logoutMessage })
      // Clear state so that refreshing the page clears the goodbye alert
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.user.role === 'admin') {
      setMessage({ type: 'error', text: 'Access denied. Please use the Admin Console.' })
      logout()
      refreshCaptcha()
      return
    }
